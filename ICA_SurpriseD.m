...
for block = blocks
    ...
    artifact_HMJ_all(find(block == blocks)).HMJ = artifact_HMJ;
    artifact_blink_all(find(block == blocks)).blink = artifact_blink;
    artifact_sacc_all(find(block == blocks)).sacc = artifact_sacc;
    sinfo_old_all(find(block == blocks)).sold = sampleinfo_old;
    fs_old_all(find(block == blocks)).fs = fsample_old;
    sinfo_new_all(find(block == blocks)).snew = sampleinfo_new;
    sinfo_HMJ_all(find(block == blocks)).sHMJ = sampleinfo_oldHMJ;
    ...
    if block == blocks(1)
        ...
        sinfo_adjusted_all(find(block == blocks)).sinfo = data.sampleinfo;
    elseif block > blocks(1) % Adjust/Offest all blocks > 1
        ...
        sinfo_adjusted_all(find(block == blocks)).sinfo = data.sampleinfo + sample_counter;
    end
...
cd('/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/data_clean/')
name = ['/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/data_clean/' ID(1:7)];
...
fprintf('\n ---------- Cleaning up -----------\n')
clearvars -except data ID filein name curr_folder

% cd('/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/data_clean/')
% name = ['/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/data_clean/' ID(1:7)];
% cd(name)
% datastore = ['data_clean_' ID '.mat'];
% load(datastore);
%%
% =====================================================================
%   8  ICA
...
try
    ...
    cd('/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/comp_ICA/')
