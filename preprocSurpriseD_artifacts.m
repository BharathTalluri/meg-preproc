...
for block = firstBl:lastBl
    ...
    cnt = 1;
    f1 = figure('vis', 'off');clf;
    f2 = figure('vis', 'off');clf;
    ...
    % -002 = Xgaze, -003 = Ygaze, -004 = pupil
    ch_mapping(1) = find(strcmp(data.label,'UADC002'));
    ch_mapping(2) = find(strcmp(data.label,'UADC003'));
    ch_mapping(3) = find(strcmp(data.label,'UADC004'));
    
    [data.trial{1}(ch_mapping(1),:), data.trial{1}(ch_mapping(2),:), ~] = eye_voltage2gaze(data.trial{1}(ch_mapping,:), ranges, screen_x, screen_y, ch_mapping);
    
    % plot gaze and pupil timecourses for this block
    set (0, 'CurrentFigure' , f2);
    subplot(3,3,[1,2]);hold on;
    plot(data.trial{1}(ch_mapping(1), :));
    set (gca, 'XLimSpec' , 'Tight' );
    title('Xgaze');
    subplot(3,3,[4,5]);hold on;
    plot(data.trial{1}(ch_mapping(2), :));
    set (gca, 'XLimSpec' , 'Tight' );
    title('Ygaze');
    subplot(3,3,[7,8]);hold on;
    plot(data.trial{1}(ch_mapping(3), :));
    set (gca, 'XLimSpec' , 'Tight' );
    title('Pupil');
    subplot(3,3,9);hold on;
    scatter(diff(data.trial{1}(ch_mapping(1), :)), diff(data.trial{1}(ch_mapping(2), :)));
    EquateAxis;xlabel('Xgaze velocity');ylabel('Ygaze velocity');
    ...
    cfgres             = [];
    cfgres.bsfilter    = 'yes';
    cfgres.bsfreq      = [49 51; 99 101; 149 151];
    data            = ft_preprocessing(cfgres, data);
    ...
    if info_EL_blocks(idx_file,5) == 1
        ...
        cfgres = [];
        cfgres.continuous = 'yes';
        cfgres.artfctdef.zvalue.channel = 'UADC003'; % Ygaze
        cfgres.artfctdef.zvalue.trlpadding = 0;
        cfgres.artfctdef.zvalue.fltpadding = 0;
        cfgres.artfctdef.zvalue.artpadding = 0.05;
        
        % Algorithmic parameters
        cfgres.artfctdef.zvalue.bpfilter = 'no';
        cfgres.artfctdef.zvalue.hilbert = 'no';
        
        % Set cutoff
        ygaze_cutoff = 200;  % cutoff in absolute pixels to be used for blink detection (anything below is blink)
        cfgres.artfctdef.zvalue.cutoff = -(ygaze_cutoff-mean(data.trial{1}(ch_mapping(2),:)))/std(data.trial{1}(ch_mapping(2),:));
        cfgres.artfctdef.zvalue.interactive = 'no';
        data.trial{1}(ch_mapping(2),:) = data.trial{1}(ch_mapping(2),:).*-1;
        [~, artifact_Ygaze] = ft_artifact_zvalue(cfgres, data);
        data.trial{1}(ch_mapping(2),:) = data.trial{1}(ch_mapping(2),:).*-1;
    else
        % --- GETTING BLINKS BASED ON VEOG
        cfgres.artfctdef.zvalue.channel = 'EEG057'; % VEOG
        cfgres.artfctdef.zvalue.trlpadding = 0;
        cfgres.artfctdef.zvalue.fltpadding = 0;
        cfgres.artfctdef.zvalue.artpadding =  0.05;
        
        % Algorithmic parameters
        cfgres.artfctdef.zvalue.bpfilter = 'yes';
        cfgres.artfctdef.zvalue.bpfilttype = 'but';
        cfgres.artfctdef.zvalue.bpfreq = [1 15];
        cfgres.artfctdef.zvalue.bpfiltord = 4;
        cfgres.artfctdef.zvalue.hilbert = 'yes';
        
        % Set cutoff
        cfgres.artfctdef.zvalue.cutoff = 2;
        cfgres.artfctdef.zvalue.interactive = 'no';
        [~, artifact_Ygaze] = ft_artifact_zvalue(cfgres, data);
    end
    
    cfgres = [];
    cfgres.artfctdef.reject = 'partial';
    cfgres.artfctdef.eog.artifact = artifact_Ygaze;
    ...
    cfgres                              = [];
    cfgres.continuous                   = 'yes'; % data has been epoched
    
    % channel selection, cutoff and padding
    cfgres.artfctdef.zvalue.channel     = {'MEG'}; % make sure there are no NaNs
    cfgres.artfctdef.zvalue.trlpadding  = 0;
    cfgres.artfctdef.zvalue.fltpadding  = 0;
    cfgres.artfctdef.zvalue.artpadding  = 0.1;
    cfgres.artfctdef.zvalue.interactive = 'no';
    
    % Algorithmic parameters
    cfgres.artfctdef.zvalue.bpfilter    = 'yes';
    cfgres.artfctdef.zvalue.bpfreq      = [110 140];
    cfgres.artfctdef.zvalue.bpfiltord   = 9;
    cfgres.artfctdef.zvalue.bpfilttype  = 'but';
    cfgres.artfctdef.zvalue.hilbert     = 'yes';
    cfgres.artfctdef.zvalue.boxcar      = 0.2;
    cfgres.artfctdef.zvalue.cutoff      = 20;  % THIS MAY NEED TO BE CHECKED AND ADJUSTED FOR DIFFERENT PARTICIPANTS
    [~, artifact_Muscle]             = ft_artifact_zvalue(cfgres, data);
    ...
    cd('/mnt/homes/home024/btalluri/confirmation_spatial/data/meg/analysis/preICA_artifactMatrices/')
    ...
    % Save the invisible figures
    set (0, 'CurrentFigure' , f1);
    figOverview = ['Overview_' ID '_Block_' num2str(block)];
    saveas(gca,figOverview,'png')
    
    set (0, 'CurrentFigure' , f2);
    figOverview = ['Gaze_Pupil_EL_' ID '_Block_' num2str(block)];
    saveas(gca,figOverview,'png')
